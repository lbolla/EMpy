...
from EMpy.utils import cond, warning, BinaryGrating, SymmetricDoubleGrating, AsymmetricDoubleGrating, Layer
# import Gnuplot
        ...
        for iwl, wl in enumerate(self.wls):
            ...
            for nlayer in xrange(nlayers-2, 0, -1):  # internal layers
                ...
                if S.absolute(K[2]/k) > 1e-10:
                    ...
                    H_S_reduced = (ky/k)**2 * I + kx[:,S.newaxis]/k * linsolve(E, kx[:,S.newaxis]/k * E11) - E11
...
class AnisotropicRCWA(RCWA):
    ...
    def solve(self, wls):
        ...
        for iwl, wl in enumerate(self.wls):
            ...
            b = S.r_[u[0]*dlt, u[1]*dlt, (k1[1]/k*u[2]-k1[2]/k*u[1])*dlt, (k1[2]/k*u[0]-k1[0]/k*u[2])*dlt]
            Ky_CRO_1 = ky/k * CRO_1
            Ky_CRE_1 = ky/k * CRE_1

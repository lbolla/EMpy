...
#            ntot[:,i] = l.mat.n(self.wls,l.mat.T0)
        ...
        for iwl, wl in enumerate(self.wls):
            ...
            Ds = [[1.,1.],[n[-1]*S.cos(theta[-1]),-n[-1]*S.cos(theta[-1])]]
        ...
        def find_roots(wl, epsilon, alpha, beta):
            ...
            for i,g in enumerate(gamma):
                ...
                v[i,:] = [(K[1,1]-alpha2-g**2)*(K[2,2]-alpha2-beta2) - (K[1,2]+beta*g)**2, \
                          (K[1,2]+beta*g)*(K[2,0]+alpha*g) - (K[0,1]+alpha*beta)*(K[2,2]-alpha2-beta2), \
...
#         g('set yrange [0:1]')
#         g('set data style linespoints')
#         g.plot(Gnuplot.Data(self.wls, self.R[0,0,:], with_ = 'linespoints', title = 'R_00'), \
#                Gnuplot.Data(self.wls, self.R[1,0,:], with_ = 'linespoints', title = 'R_10'), \
#                Gnuplot.Data(self.wls, self.R[0,1,:], with_ = 'linespoints', title = 'R_01'), \
#                Gnuplot.Data(self.wls, self.R[1,1,:], with_ = 'linespoints', title = 'R_11'), \
#                Gnuplot.Data(self.wls, self.T[0,0,:], with_ = 'linespoints', title = 'T_00'), \
#                Gnuplot.Data(self.wls, self.T[1,0,:], with_ = 'linespoints', title = 'T_10'), \
#                Gnuplot.Data(self.wls, self.T[0,1,:], with_ = 'linespoints', title = 'T_01'), \
#                Gnuplot.Data(self.wls, self.T[1,1,:], with_ = 'linespoints', title = 'T_11'))
#         raw_input('press enter to close the graph...')

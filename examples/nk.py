...
## SiO2 from 2007: A=1.4764 B=0.02299 C=-0.012346 n(1550nm) = 1.4838 dep. rate = 133.908 nm/1800 sec
IBD_SiO2_DJ2007 = lambda wl: cauchy([ 1.4764 , 0.02299 , -0.012346 ], wl)
# Ta2O5 from 2007: A=2.1123 B=0.018901 C=-0.016222 n(1550nm) = 2.1123 dep.rate = 326.586 nm/2631.579 sec
IBD_Ta2O5_DJ2007 = lambda wl: cauchy([ 2.1123 , 0.018901 , -0.016222 ], wl)
# from 2011:
IBD_SiO2_DJ2011 = lambda wl: cauchy( [1.479, 0.00472, 1.3305e-5],  wl )
## SiN 12/1/2010: A=2.003 // B=0.01954 // C=3.0014e-4  (0.776233 Ang/sec)
...
def check_wl(wl, wl_low, wl_hi, func_name):
    '''Raise a warning if the specified wavelength is out of range.
    
    Parameters
    ----------
    wl: the wavelength to check
    wl_low: minimum wavelength
    wl_hi: maximum wavelength
    func_name: name of the function being called - will be inserted into Warning message.
    '''
    wlmin = wl_low
    wlmax = wl_hi
    if np.any(wl<wlmin) or np.any(wl>wlmax): 
        ErrStr = func_name + "(): Wavelength (%sum) outside of model range (%f-%f um)."%(wl,wlmin,wlmax) + "  Refractive Index value may not be accurate."
        warnings.warn(ErrStr)
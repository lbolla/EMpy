...
class Input(object):
    ...
    def __str__(self):
        ...
        if len(self.dftmonitors) > 0:
            dftmon_str += ''.join(['%g %g %g %g %g %g\n%g %g\n' % (dm[0][0],dm[0][1],dm[0][2],dm[0][3],dm[0][4],dm[0][5],dm[1][0],dm[1][1]) for dm in self.dftmonitors])

        timemon_str = '%g ! #timemonitors \n' % len(self.dftmonitors)
        if len(timemon_str) > 0:
            timemon_str += '%g %g \n %s' % (self.timemonitors_time_interval[0], 
                                            self.timemonitors_time_interval[1], 
                                            ''.join(['%g %g %g ! time_monitor #%d\n' % (s[0], s[1], s[2], iss) for iss,s in enumerate(self.timemonitors)]))
            
        return '%g %g %g %g ! dx dy dz cfl \n' \
               ...
               '%s' \
               '%s' \
    % (self.dx, self.dy, self.dz, self.cfl, \
       self.xmax, self.ymax, self.zmax, self.pmlx, self.pmly, self.pmlz, self.pmltype, self.pmlsmooth, self.pmlref, \
       self.start, self.end, self.slides, self.snapshot, \
       self.output3deps, \
       len(self.dielslices), \
       '\n'.join(['%g %g %g ! dielslice #%d' % (d[0], d[1], d[2], dd) for (dd,d) in enumerate(self.dielslices)]), \
       len(self.fieldslices), \
       '\n'.join(['%g %g %g ! fieldslice #%d' % (f[0], f[1], f[2], ff) for (ff,f) in enumerate(self.fieldslices)]), \
       len(self.dielobjs), self.bgrix, self.bgsigma, \
       ''.join(['%s %s\n' % obj for obj in self.dielobjs]), \
       self.smoothing_method, \
       len(self.sources), \
       ''.join(['%s\n%s\n%s\n%s\n' % src for src in self.sources]), \
       self.lambdamin, self.lambdamax, self.dlambda, \
       dftmon_str, \
       timemon_str, \
       )
...
class Param(object):
    ...
    def __str__(self):
        ...
        return '%g\n' \
               ...
               '%g\n' \
               '%s' \
               % (self.dx, self.dy, self.dz, self.dt, self.mx, self.my, self.mz, self.pmlx, self.pmly, self.pmlz, \
...
class FDTD(object):
    ...
    def fetch_data(self, remote_dir_='./', input_file='inp.txt', param_file='param', directory_='./'):
        remote_dir = fixdir(remote_dir_)
        directory = fixdir(directory_)
        # input file
        os.system('scp -C bollalo001@pico:' + remote_dir + '/' + input_file + ' ' + directory)
        # param file
        os.system('scp -C bollalo001@pico:' + remote_dir + '/' + param_file + ' ' + directory)
        # fieldslices, flux and time sensors
        os.system('scp -C bollalo001@pico:' + remote_dir + '/[EHeh]*_*' + ' ' + directory)
        # dielslices
    ...
    def load(self, directory_='./', input_file='inp.txt', param_file='param', remote_dir_=''):
    ...
    def load_input_file(self, directory_='./', filename='inp.txt'):
        ...
        (inp.dx, inp.dy, inp.dz, inp.cfl) = numpy.fromstring(strip_comment(f.readline()), sep = ' ')
        ##OKKIO
        ...
        (inp.xmax, inp.ymax, inp.zmax, inp.pmlx, inp.pmly, inp.pmlz) = numpy.fromstring(tmp[:tmp_idx], sep=' ')
        (inp.pmlsmooth, inp.pmlref) = numpy.fromstring(tmp[tmp_idx + 1:], sep=' ')
        (inp.start, inp.end, inp.slides, inp.snapshot) = numpy.fromstring(strip_comment(f.readline()), sep=' ')
...
##            [tmp.E1, tmp.H1, tmp.E2, tmp.H2] = map(lambda x: x[0::2] + 1j * x[1::2], [tmp.E1, tmp.H1, tmp.E2, tmp.H2])
    ...
    def viz2D(self, filename, directory_='./', const_dir = 'z', logplot = False):
        ...
        if self.param is None:
            self.load_param(directory)
        x = numpy.linspace(self.param.dx/2., self.param.dx*self.param.mx - self.param.dx/2., self.param.mx)
        y = numpy.linspace(self.param.dy/2., self.param.dy*self.param.my - self.param.dy/2., self.param.my)
        z = numpy.linspace(self.param.dz/2., self.param.dz*self.param.mz - self.param.dz/2., self.param.mz)
        if const_dir == 'x':
            n1 = self.param.my
            n2 = self.param.mz
            x1 = y
    ...
    def memory(self):
        ...
        # epsx, epsy, epsz
        dynamic_alloc_memory = dynamic_alloc_memory + 3*(self.param.mx+2*self.input.pmlx)*(self.param.my+2*self.input.pmly)*(self.param.mz+2*self.input.pmlz)*size_of_real
        # sigma
        dynamic_alloc_memory = dynamic_alloc_memory + 2*(self.param.mx+2*self.input.pmlx)*(self.param.my+2*self.input.pmly)*(self.param.mz+2*self.input.pmlz)*size_of_real
        # cex, cmx
        dynamic_alloc_memory = dynamic_alloc_memory + 2*2*(self.param.mx+2*self.input.pmlx)*size_of_real
        # cey, cmy
        dynamic_alloc_memory = dynamic_alloc_memory + 2*2*(self.param.my+2*self.input.pmly)*size_of_real
        # cez, cmz
        dynamic_alloc_memory = dynamic_alloc_memory + 2*2*(self.param.mz+2*self.input.pmlz)*size_of_real
...
##        cmd = 'dplace -x6 ' + exe_file + ' > ' + output_file
        cmd = 'cd' + directory + '; setenv OMP_NUM_THREAD' + str(ncpu) + 'dplace -x6 ' + exe_file + ' > ' + output_file
        if bg:
            cmd += '&'
        if remote:
            cmd = 'ssh pico "' + cmd + '"'
    ...
    def __str__(self):
        ...
        return 'INPUT\n%s\nPARAM\n%s\nSENSORS\n%s\n' % (self.input, self.param, self.sensors)
